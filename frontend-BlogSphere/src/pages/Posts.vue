		...
		<Modal :isOpen="isModalOpen" @close="closeModal">
			<form @submit.prevent="savePost">
				...
				<div class="mb-3">
					...
					<select v-model="form.user_id" class="form-control" id="user" required>
				...
				<div class="mb-3">
					...
					<select v-model="form.tags" class="form-control" id="tags" multiple required>
...
export default {
	...
	methods: {
		...
		async deletePost(id) {
			try {
				// Faz a requisição para excluir o post no backend
				await api.delete(`/posts/${id}`);

				// Atualiza a lista local, removendo o post excluído
