			...
			<tbody>
				<tr v-for="tag in filteredTags" :key="tag.id">
					...
					<td class="text-center">
						<button @click="editTag(tag)" class="btn btn-warning btn-sm me-2">
							Editar
						</button>
...
export default {
	...
	methods: {
		...
		async deleteTag(id) {
			try {
				// Faz a requisição para excluir a tag no backend
				await api.delete(`/tags/${id}`);

				// Atualiza a lista local, removendo a tag excluída
				this.tags = this.tags.filter(tag => tag.id !== id);
				this.filteredTags = this.filteredTags.filter(tag => tag.id !== id);
				...
				alert("Não foi possível excluir a tag. Tente novamente.");
			}
		},
	},
